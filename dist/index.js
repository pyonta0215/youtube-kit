...
export const COMBINED_UNIT_COST = Object.freeze({
    channels: 1,
    videos: 1,
    playlistItems: 1,
});
...
export function createQuotaMeter() {
    return { units: 0, searchCalls: 0, calls: 0, failed: 0, by: {} };
}
/** クライアントを通さずに API を呼んだ箇所から、手で数えるための入口。 */
export function countCall(meter, method) {
    if (!meter)
        return;
    if (method === 'search')
        meter.searchCalls += 1;
    else
        meter.units += COMBINED_UNIT_COST[method];
    meter.calls += 1;
...
export class YouTubeApiError extends Error {
    ...
    status;
    reason;
    constructor(method, kind, details = {}) {
...
export function parseChannelRef(raw) {
    ...
    if (embeddedId)
        return { kind: 'id', value: embeddedId };
    if (video)
        return { kind: 'video', value: video };
...
export function parseVideoRef(raw, options = {}) {
    const s = String(raw ?? '').trim();
    if ((options.allowBareId ?? true) && VIDEO_ID.test(s))
        return s;
    const match = /[?&]v=([\w-]{11})(?![\w-])/.exec(s) ??
...
export function parseIsoDuration(iso) {
    ...
    if (!match || iso === 'P' || iso === 'PT')
        return null;
    const [days, hours, minutes, seconds] = match.slice(1).map((v) => (v ? Number(v) : 0));
 ...
 */
export function isLiveByDuration(seconds) {
    return seconds === 0;
}
/** 0 秒（ライブ）は Shorts に含めない。尺が読めないときも false */
export function isShortByDuration(seconds) {
    return seconds !== null && seconds > 0 && seconds <= SHORT_MAX_SECONDS;
}
/** 尺から形式を推定する。中央値を形式ごとに分けて取るとき、live と unknown をどちらにも混ぜないために使う */
export function videoFormatByDuration(seconds) {
    if (seconds === null)
        return 'unknown';
    if (isLiveByDuration(seconds))
        return 'live';
    return isShortByDuration(seconds) ? 'short' : 'long';
}
export function chunk(items, size = MAX_IDS_PER_REQUEST) {
    const out = [];
...
const asItems = (json) => (Array.isArray(json.items) ? json.items.map(asRecord) : []);
...
export function createYouTubeClient(options) {
    ...
    async function call(method, params) {
        if (!options.apiKey)
            throw new YouTubeApiError(method, 'no-api-key');
    ...
    return {
        ...
        async playlistVideoIds(playlistId, limit = MAX_IDS_PER_REQUEST) {
            const ids = [];
            let pageToken;
            let pages = 0;
        ...
        async searchVideos(params) {
            ...
            const hits = asItems(json).flatMap((item) => {
                const snippet = asRecord(item.snippet);
